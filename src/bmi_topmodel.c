...
static const char *input_var_units[INPUT_VAR_NAME_COUNT] = {"m h-1", "m h-1"};
...
static const char *param_var_names[PARAM_VAR_NAME_COUNT] = {
    ...
    "szm", // exponential scaling parameter for the decline of transmissivity with increase in
           // storage deficit (m)
...
int read_init_config(const char *config_file, topmodel_model *model) {
    ...
    ret = fscanf(
        model->subcat_fptr,
        "%d %d %d",
        &model->num_sub_catchments,
        &model->imap,
        &model->yes_print_output
    );
    if (ret != 3)
        return BMI_FAILURE;
    else
...
int init_config(const char *config_file, topmodel_model *model) {
    ...
    if (model->stand_alone == TRUE) {
        ...
        d_alloc(
            &model->Qobs,
            model->nstep
        ); // TODO: Consider removing this all together when framework
    ...
    tread(
        ...
        model->output_fptr,
        model->subcat,
        &model->num_topodex_values,
        &model->num_channels,
        &model->area,
        &model->dist_area_lnaotb,
        &model->lnaotb,
        model->yes_print_output,
        model->stand_alone,
        &model->cum_dist_area_with_dist,
        &model->tl,
        &model->dist_from_outlet
    );
    ...
    ret = init(
        ...
        model->yes_print_output,
        model->area,
        &model->time_delay_histogram,
        model->cum_dist_area_with_dist,
        model->dt,
        model->tl,
        model->dist_from_outlet,
        &model->num_time_delay_histo_ords,
        &model->num_delay,
        &model->szm,
        &model->t0,
        &model->chv,
        ...
        &model->hf,
        &model->dth,
        &model->stor_unsat_zone,
        &model->deficit_local,
        &model->deficit_root_zone,
        &model->szq,
        &model->Q,
        &model->sbar,
        &model->bal
...
#endif
    *time +=
        (((topmodel_model *)self->data)->current_time_step * ((topmodel_model *)self->data)->dt);
    return BMI_SUCCESS;
}
...
static int Update(Bmi *self) {
    ...
    topmod(
        topmodel->output_fptr,
        topmodel->nstep,
        topmodel->num_topodex_values,
        topmodel->yes_print_output,
        topmodel->infex,
        topmodel->dt,
        topmodel->szm,
        topmodel->stor_unsat_zone,
        topmodel->deficit_root_zone,
        topmodel->deficit_local,
        topmodel->pe,
        topmodel->rain,
        topmodel->xk0,
        topmodel->hf,
        topmodel->dist_area_lnaotb,
        topmodel->tl,
        topmodel->lnaotb,
        topmodel->td,
        topmodel->srmax,
        topmodel->contrib_area,
        topmodel->szq,
        &topmodel->Qout,
...
static int Finalize(Bmi *self) {
    ...
    if (self) {
        ...
        if (model->yes_print_output == TRUE &&
            model->stand_alone == TRUE) { // && model->stand_alone == TRUE){
...
static int Get_value_ptr(Bmi *self, const char *name, void **dest) {
    ...
    // qof
    if (strcmp(name, "land_surface_water__domain_time_integral_of_overland_flow_volume_flux") ==
        0) {
        topmodel_model *topmodel;
        topmodel = (topmodel_model *)self->data;
    ...
    // sump
    if (strcmp(name, "land_surface_water__domain_time_integral_of_precipitation_volume_flux") ==
        0) {
        topmodel_model *topmodel;
        topmodel = (topmodel_model *)self->data;
...
static int Set_value(Bmi *self, const char *name, void *array) {
    ...
    if (nameIsCalibQParam || nameIsCalibWBParam || strcmp(name, "srmax") == 0 ||
        strcmp(name, "td") == 0) {
