...
extern void topmod(
    ...
    /*****************************************************************
      ...
      equal to zero.
      REMEMBER SBAR,deficit_local AND deficit_root_zone VALUES ARE DEFICITS; stor_unsat_zone IS A
    STORAGE.
    ******************************************************************/
    ...
    *sump += *p; /* BMI Adaption: *sump now as pointer var; incl in model struct */
    ...
    for (ia = 1; ia <= num_topodex_values; ia++) {
        ...
        if (deficit_local[ia] > 0.0) {
            ...
            if (uz > stor_unsat_zone[ia])
                uz = stor_unsat_zone[ia];
            stor_unsat_zone[ia] -= uz;
            if (stor_unsat_zone[ia] < 0.0000001)
        ...
        if (*ep > 0.0) {
            ...
            if (ea > (srmax - deficit_root_zone[ia]))
                ea = srmax - deficit_root_zone[ia];
        ...
        *sumae += ea * acf; /* BMI Adaption: *sumae now as pointer var; incl in model struct */
        ...
        contrib_area[it] = acm;
        if (acm > max_contrib_area)
            max_contrib_area = acm;
    ...
    *sumq += Q[it];
    ...
    if (yes_print_output == TRUE && in <= current_time_step && stand_alone == TRUE) {
        Log(INFO,
            "%d %6.4e %6.4e %6.4e %6.4e %6.4e %6.4e %6.4e\n",
            current_time_step,
            (*p),
            (*ep),
            Q[it],
...
extern void water_balance(
    ...
    Log(DEBUG, "   SUMP       SUMAE      SUMQ       SUMRZ      SUMUZ      SBAR        BAL\n");
    Log(DEBUG,
        "%6.3e  %6.3e  %6.3e  %6.3e  %6.3e  %6.3e  %6.3e\n",
        (*sump),
        (*sumae),
        (*sumq),
        (*sumrz),
    ...
    if (yes_print_output == TRUE && stand_alone == TRUE) {
        ...
        Log(INFO, "   SUMP       SUMAE      SUMQ       SUMRZ      SUMUZ      SBAR        BAL\n");
        Log(INFO,
            "%6.3e  %6.3e  %6.3e  %6.3e  %6.3e  %6.3e  %6.3e\n",
            (*sump),
            (*sumae),
...
#define TOPMODEL_CHECK_FGETS(x) \
    do {                        \
        str_ret = x;            \
        if (str_ret == NULL)    \
            return 1;           \
    } while (0)
#define TOPMODEL_CHECK_FSCANF(k, x) \
    do {                            \
        ret = x;                    \
        if (ret != k)               \
            return 1;               \
...
extern int tread(
    ...
    *tl = 0.0;
    ...
    for (j = 1; j <= (*num_channels); j++) {
        TOPMODEL_CHECK_FSCANF(
            2,
            fscanf(subcat_fptr, "%lf %lf", &(*cum_dist_area_with_dist)[j], &(*dist_from_outlet)[j])
        );
    }
 ...
 * 	the 1-based indexing was directly ported as well, so the tch buffer was over allocated by
 one,
 ...
 * @params[in] CALIBRATABLE chv, pointer of type double and length one, the average channel velocity
 (m/hr)
 * 	for the main channel (BChoat)
 * @params[in] CALIBRATABLE rv, pointer of type double and length one, average internal overland
 flow routing velocity
 * @params[in] dt, double of length one, timestep in hours

 * @params[out] tch, double* of length num_channels, holds histogram ordinates for each channel
 (BChoat)
 ...
 * @params[out] num_time_delay_histo_ords, pointer of type int, number of time delay histogram
 ordinates
 * @params[out] num_delay, pointer of type int, number of time steps lag (delay) in channel within
 * 	catchment to outlet.
 * @parms[out], time_delay_histogram, pointer of size and length max_time_delay_ordinates and type
 double,
 ...
 * 	entire catchment
 * @params[in] num_time_delay_histo_ords, pointer of type int, number of time delay histogram
 * ordinates, output from calc_time_delay_histogram
 * @parms[in], time_delay_histogram, double pointer of type double, time lag of outflows due to
 * channel routing, output from calc_time_delay_histogram
 *
 * @params[out] Q, pointer of type double and length num_delay+num_time_delay_histo_ords, simulated
 * discharge
 */
 ...
 * 	(i.e., number of A/TANB ordinates), defines the size of stor_unsat_zone, deficit_root_zone,
 and deficit_local
 ...
 * @params[in] Q0, pointer of type double, initial subsurface flow per unit area
 * @params[in] CALIBRATABLE t0, pointer of type double, downslope transmissivity when soil is just
 saturated
 ...
 * @params[out] deficit_local, double pointer of type double of size num_topodex_values, local
 storage (or saturation)
 * 	deficit
 * @params[out] deficit_root_zone, double pointer of type  double of size num_topodex_values, root
 zone storage deficit
 ...
 * @params[in] output_fptr, FILE pointer, file to which output will be written (e.g.,
 topmod-cat.out)
 ...
 * @params[in] yes_print_output, int (boolean), 1 = TRUE; write output files, 0 = FALSE; do not
 write
 * @params[in] area, double between 0 and 1 defining catchment area as ratio of
 * 	entire catchment
 * @parms[in], time_delay_histogram, double pointer of type double, time lag of outflows due to
 ...
 * @params[in], cum_dist_area_with_dist, pointer of type double, cumulative distribution of area
 with
 * 	dist_from_outlet
 * @params[in] dt, double, timestep in hours
 * @params[in] tl, double, not well defined, but related to time lag
 ...
 * 	of transmissivity with increase in storage deficit
 * @params[out] CALIBRATABLE t0, pointer of type double, downslope transmissivity when soil is just
 saturated
 * 	to the surface. 'input as LN(t0)
 * @params[out] CALIBRATABLE td, pointer of type double, unsaturated zone time delay for recharge to
 the
 ...
 * @params[out] CALIBRATABLE rv, double of length one, average internal overland flow routing
 velocity
 * @params[out] CALIBRATABLE srmax, pointer of type double, maximum root zone storage deficit
 provided in params.dat
 * 	Only read in within init(), not used.
 ...
 * @params[out] infex, pointer of type int (boolean), 1 = TRUE; call subroutine to do infiltration
 excess calcs,
 ...
 * @params[out] deficit_local, double pointer of type double of size num_topodex_values, local
 storage (or saturation)
 * 	deficit
 * @params[out] deficit_root_zone, double pointer of type double of size
 num_topodex_valuesnum_topodex_values,
...
extern int init(
    ...
    ******************************************/
    ...
    TOPMODEL_CHECK_FSCANF(
        ...
        fscanf(
            in_param_fptr,
            "%lf %lf %lf %lf %lf %lf %lf %lf %d %lf %lf %lf",
            szm,
            t0,
            td,
            chv,
